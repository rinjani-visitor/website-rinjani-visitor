  ...
  const updateUser = async (event) => {
    ...
    if (file) {
      ...
      const body = {
        name: name === data?.name ? undefined : name,
        phoneNumber:
          phoneNumber === data?.phoneNumber ? undefined : phoneNumber,
        country: country === data?.country ? undefined : country,
      };
    ...
    <div className="">
      ...
      <form className="space-y-4" onSubmit={updateUser}>
        ...
        <InputFormSign
          ...
          type={`tel`}
          value={phoneNumber}
          method={handlerPhoneNumber}
        />
        <SelectCountry value={country} method={handlerCountry} />
        {infoSuccess}
        <button
          type="submit"
          className={`w-full font-medium rounded-md h-10 bg-rinjaniVisitor-green text-white `}
        >
          {isLoad ? (
